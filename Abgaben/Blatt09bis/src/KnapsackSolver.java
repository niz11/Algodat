...
public class KnapsackSolver {
	...
	public Knapsack solveKnapsackGreedySmart(Knapsack k, LinkedList<Item> items){
		...
		for (int i=0 ; i<items.size(); i++) {
			...
			int Weight1 = items.get(i).getWeight();
			double Weight = items.get(i).getWeight();
			newValue = Value/Weight;
			newValue = newValue *100000;
			//System.out.println(newValue);
			newIntValue = (int) newValue;
			//System.out.println(items.get(i).getIndex()+ " Value- " +newIntValue);
			Item e = new Item(items.get(i).getIndex(), Weight1 ,newIntValue );
			Sorteditems.add(e);
			newIntValue = 0;
			newValue=0;
