...
public class Network implements INetwork {
	...
	public List<Edge> findMinCut() {
		...
		Node source = findSource();
		LinkedList<Node> nodelist = findReachableNodes(source);
		for(Node e : nodelist){
			LinkedList<Edge> edgeCheck = e.getIncidentEdges();
			for(Edge kante : edgeCheck){
				if (kante.capacity== 0)
					cutEdges.add(kante);
			}
		}
	...
	public LinkedList<Node> findReachableNodes(Node startNode) {
		...
		int totalFlow = fordFulkerson();
		 nodeList = breadthFirstSearch(startNode);
		...
		if (startNode == null || !nodes.values().contains(startNode)) {
			...
			while (!queue.isEmpty()) {
				...
				for (Node neighbor : current.getSuccessorNodes()) {
					...
					/*
					 * I ,Nizan , Changed here the second bedinung from the if , to take it back , just delete the && and the second term
					 */
					if ((neighbor.status == Node.WHITE) && (current.getEdgeTo(neighbor).capacity>0)) {
