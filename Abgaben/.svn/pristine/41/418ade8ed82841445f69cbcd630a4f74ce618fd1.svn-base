...
public class Matcher implements IMatcher {
	...
	protected void computeDistance() throws RuntimeException {
	    ...
	    this.distance= this.accumulatedDistance[SpalteIndex][zeileIndex];
	}
	...
	protected void computeMatchingPath() throws RuntimeException {
		...
		/*
		 * the nummbers of row and colums in the accumalteddistanceMatrix
		 */
	    ...
	    /*
	     * a loop that will go on the Matrix and collect all the correct pairs , diagonale is in priority
	     * 		 0.0 , inf, inf, inf, inf,inf}, 
				...
				Pairs in the MAtrix = {{1,1},{2,1},{3,2},{4,3},{5,4},{5,5}};
				expectedPath = {{0,0},{1,0},{2,1},{3,2},{4,3},{4,4}};
				
				zeileIndex= 5
				SpalteIndex = 5
	     */
	    	
	    	while ((zeileIndex > 0) && (SpalteIndex>0) )
	    			{
					if ((this.accumulatedDistance[zeileIndex-1][SpalteIndex-1] <= this.accumulatedDistance[zeileIndex][SpalteIndex-1]) && (this.accumulatedDistance[zeileIndex-1][SpalteIndex-1] <= this.accumulatedDistance[zeileIndex-1][SpalteIndex] )) {
						Pair newPair = new Pair(zeileIndex-2,SpalteIndex-2);
						matchingPath.add(newPair);
						zeileIndex--;
						SpalteIndex--;
						...
						Pair newPair = new Pair(zeileIndex-1,SpalteIndex-2);
						matchingPath.add(newPair);
						//zeileIndex--;
						SpalteIndex--;
					}
					...
					{
						Pair newPair = new Pair(zeileIndex-2,SpalteIndex-1);
						matchingPath.add(newPair);
						//SpalteIndex--;
						zeileIndex--;
						//matchingPath.add(i, newPair);
					}
					if(zeileIndex==1 && SpalteIndex==1 ) break;
					
				}
	    	/*
	    	 * need to rotare my Path , so it will start from 0.0 to the end . ausklammert are my path printing before and after the rotate , to see if it worked
	    	 */
	    	/*
	    	for(int s= 0; s<matchingPath.size(); s++) {
	    		System.out.println(matchingPath.get(s) + " first Path");
	    	}
	    	*/
	    	for(int s= 0; s<matchingPath.size(); s++) {
	    		Pair newPair = matchingPath.get(matchingPath.size()-1);
	    		matchingPath.remove(matchingPath.size()-1);
	    		matchingPath.add(s, newPair);
	    	}
	    	/*
	    	for(int s= 0; s<matchingPath.size(); s++) {
	    		System.out.println(matchingPath.get(s) + " second Path");
	    	}
	    	*/
