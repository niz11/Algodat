...
//			if (neighbor != null) {n.addEdge(neighbor, 1);}
	...
	public void populateAStar(CellNode startCNode, CellNode targetCNode) {
		...
		/*
		 * First I konstrukt my heuristic values to each node
		 */
		HeuristicEuclidean heuristic = new HeuristicEuclidean(targetCNode);
		for (CellNode n : cellnodes)
			heuristic.estimateDistanceToGoal(n);
		
		/*
		 * Starting A* - Dijekstra + heuristic Value
		 */
		
		PriorityQueue<CellNode> distanceQueue = new PriorityQueue<CellNode>();
				...
				while (!distanceQueue.isEmpty()) {
					...
					for (Edge incidentEdge : current.getIncidentEdges()) {
						...
						if (neighbor.status == Node.WHITE) {
							...
							neighbor.distance = (int) (current.distance + incidentEdge.weight + neighbor.distanceToGoalEstimate);
	...
	public List<Node> getShortestPathAStar(Node startNode, Node targetNode) {
		/*
		 * Chosing HeuristicEuclidean , because in a picture where all the edges have the same values , the Manhaten 
		 * 0,0* 6* * *      0,0* * * *     
		 * 6* 6* * *        * sqrt(18)* * *
		 * * * * *          * * * *
		 * * * * 4,4*       * * * 4,4*
		...
		CellNode startNode1 = getCellNode(startNode);
		CellNode targetNode1 = getCellNode(targetNode);
		if (startNode1== targetNode1 ) {
			path.add(targetNode1);
			return path;
		}
		...
		this.populateAStar(startNode1, targetNode1);
